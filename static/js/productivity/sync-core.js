...
// src/db.ts
var ProductivityDB = class extends import_wrapper_default {
  notes;
  outbox;
  meta;
  constructor() {
...
var _listener = null;
function setLocalWriteListener(cb) {
  _listener = cb;
}
function notifyLocalWrite() {
  if (_listener) _listener();
}
async function syncedUpsert(record) {
...
async function syncedDelete(id) {
  const editedAt = nowIso();
  await db.transaction("rw", [db.notes, db.outbox], async () => {
...
// src/notesRepo.ts
var META = /* @__PURE__ */ new Set(["_dirty", "_baseRev", "_editedAt"]);
function clean(row) {
  const out = {};
...
function newId() {
  return typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : "n_" + Math.random().toString(36).slice(2);
}
var notesRepo = {
  async list() {
  ...
  subscribe(cb) {
    const h = () => cb();
    db.notes.hook("creating", h);
    db.notes.hook("updating", h);
    db.notes.hook("deleting", h);
    return () => {
      db.notes.hook("creating").unsubscribe(h);
      db.notes.hook("updating").unsubscribe(h);
      db.notes.hook("deleting").unsubscribe(h);
    };
  }
};

// src/engine.ts
var META2 = /* @__PURE__ */ new Set(["_dirty", "_baseRev", "_editedAt"]);
var BACKOFF_MIN = 2e3;
var BACKOFF_MAX = 6e4;
function stripMeta(row) {
  const out = {};
  for (const [k, v] of Object.entries(row)) if (!META2.has(k)) out[k] = v;
  return out;
}
function createSyncClient(opts = {}) {
  ...
  async function pushOnce() {
    ...
    for (const r of results) {
      ...
      await db.notes.put({ ...r.record, _dirty: 0, _baseRev: r.rev, _editedAt: r.record.updated_at ?? (/* @__PURE__ */ new Date(0)).toISOString() });
    }
  }
  ...
  function start() {
    setLocalWriteListener(() => {
      clearTimeout(timer);
      timer = setTimeout(() => void syncOnce(), 3e3);
    });
...
function parseCapture(text) {
  ...
  while ((m = TOKEN.exec(text)) !== null) {
    ...
    if (m[1]) {
      project = tok.slice(1);
      segments.push({ text: tok, kind: "project" });
    } else if (m[2]) {
      bucket = tok.toLowerCase();
      segments.push({ text: tok, kind: "bucket" });
    } else if (m[3]) {
      dueTime = tok.toLowerCase();
      segments.push({ text: tok, kind: "time" });
    } else if (m[4]) {
      urgency = tok.length >= 2 ? 2 : 1;
      segments.push({ text: tok, kind: "urgency" });
    }
